      ...
      >
        <div className="relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all duration-500 hover:shadow-xl hover:shadow-black/5">
          ...
          <div className="relative z-10 text-center">
            ...
            <h3 className="text-lg font-bold text-black tracking-tight">{member.name}</h3>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-3">{member.role}</p>
            ...
            <p className="mb-5 text-sm text-gray-500 leading-relaxed line-clamp-2">{member.bio}</p>

            {/* skills */}
            <div className="mb-6 flex flex-wrap justify-center gap-1.5">
              {member.skills.map((skill) => (
                <span key={skill} className="rounded-full bg-gray-50 border border-gray-100 px-2.5 py-0.5 text-[10px] font-medium text-gray-500">
                  {skill}
                </span>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-3">
              {[
                ...
                <a key={i} href={href} className="text-gray-300 hover:text-black transition-colors">
      ...
      <section className="relative pt-32 pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial="hidden" animate="visible" variants={containerVariants}>
            <motion.div variants={itemVariants} className="mb-6 inline-flex items-center gap-2 rounded-full border border-gray-100 bg-gray-50/50 px-4 py-1 text-[11px] font-bold uppercase tracking-widest text-gray-400">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
              Our Story
            </motion.div>
            <motion.h1 variants={itemVariants} className="text-5xl md:text-7xl font-bold tracking-tight mb-8 leading-[0.95]">
              We build tools for the <span className="text-gray-300 italic font-medium">next generation</span> of shopping.
            </motion.h1>
            <motion.p variants={itemVariants} className="text-lg text-gray-500 max-w-2xl mx-auto leading-relaxed">
              UnityShop connects millions of buyers and sellers worldwide with a focus on trust, speed, and simplicity.
            </motion.p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="border-y border-gray-100 bg-gray-50/30">
        <div className="max-w-6xl mx-auto px-6 py-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, i) => (
              <motion.div key={i} custom={i} initial="hidden" whileInView="visible" variants={fadeUp} viewport={{ once: true }} className="text-center md:text-left">
      ...
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16 text-center md:text-left">
            <h2 className="text-3xl font-bold tracking-tight mb-4">The people behind the pixels</h2>
            <p className="text-gray-500 max-w-md">Our team is a small group of thinkers and makers dedicated to building the best commerce experience.</p>
          </div>
