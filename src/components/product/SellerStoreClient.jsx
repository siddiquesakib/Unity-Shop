...
import { FiStar, FiShield, FiPackage, FiTruck, FiClock } from "react-icons/fi";
...
export default function SellerStoreClient({ sellerName }) {
  ...
  // If the prop got through as the string "undefined", clean it up
  ...
  const fetchSellerAndProducts = useCallback(async () => {
    ...
    try {
      ...
      if (userRes.ok) {
        ...
        // Fallback if no full profile info
        setSeller({ name: activeSellerName });
      }
