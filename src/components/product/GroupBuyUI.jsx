...
import { FiUsers, FiClock, FiShare2, FiCheckCircle, FiCopy, FiLoader } from "react-icons/fi";
...
export default function GroupBuyUI({ productId, user, formatPrice }) {
    ...
    const leaveGroup = async (groupId) => {
        ...
        if (!confirm("Are you sure you want to leave/cancel this group buy?")) return;
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white">
                        <FiUsers size={20} />
                    </div>
                    <div>
                        <p className="font-bold text-gray-900 text-sm">Team Discount</p>
                        <p className="text-indigo-600 text-xs font-medium">Save 20% with friends</p>
                    </div>
                </div>
                <button
                    onClick={startGroup}
                        ...
                        >
                            ...
                            <div className="flex gap-2">
                                {group.members.includes(user?._id) ? (
                                    ...
                                    >
                                        {leavingId === group._id ? <FiLoader className="animate-spin" /> : <FiX size={14} />}
                                        {group.creatorId === user?._id ? "Cancel Group" : "Leave Group"}
                                    </button>
                                ) : (
                                    <button
                                    ...
                                    >
                                        {joiningId === group._id ? <FiLoader className="animate-spin mx-auto" /> : "Join Team"}
                                    </button>
                                )}
                                <button
...
function FiUser({ size }) {
    return (
        <svg
            stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" height={size} width={size}
        >
